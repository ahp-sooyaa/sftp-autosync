...
export class ProjectWatcher {
  ...
  async #handle(project, absolutePath) {
    ...
    if (st.isFile()) {
      ...
      if (shouldSkipUnchanged(this.#contentHashes.get(absolutePath), hash)) {
...
/** SHA-256 hex digest of file bytes (used to skip same-content rewrites). */
export function contentHash(bytes) {
  return createHash("sha256").update(bytes).digest("hex");
}

/** True when we already successfully uploaded these exact bytes. */
