...
import { daemonLogDir, migrateLegacyConfig } from "./paths.js";
...
export async function installLaunchAgent({
  ...
  const logDir = daemonLogDir(home);
...
/** Reload the existing LaunchAgent without rewriting the plist. */
export async function reloadLaunchAgent({
  ...
  if (!exists(plistPath)) {
    throw new Error("LaunchAgent not installed. Run: sftp-autosync init --launchd");
  }

  const unloaded = (await launchctl("unload", plistPath)) === 0;
  let load = await launchctl("load", plistPath);
  if (load !== 0) {
    load = await launchctl("load", plistPath);
    if (load !== 0) {
      if (unloaded) {
        throw new Error(
          "launchctl load failed after unload; sync daemon is stopped. Run: sftp-autosync init --launchd",
        );
      }
      throw new Error(
        "launchctl load failed; LaunchAgent may still be running with the previous config. Run: sftp-autosync init --launchd",
      );
    }
  }
