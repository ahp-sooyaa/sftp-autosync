...
/** macOS daemon log directory (launchd stdout/stderr). */
export function daemonLogDir(home = homedir()) {
  return join(home, "Library", "Logs", "sftp-autosync");
}

export function daemonOutLogPath(home = homedir()) {
  return join(daemonLogDir(home), "out.log");
}

export function daemonErrLogPath(home = homedir()) {
  return join(daemonLogDir(home), "err.log");
}
