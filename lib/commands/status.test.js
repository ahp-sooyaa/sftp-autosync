...
describe("formatProjectStatus", () => {
  ...
  test("includes sticky lastError and pending retry summary", () => {
    const root = mkdtempSync(join(tmpdir(), "sftp-status-pending-"));
    try {
      mkdirSync(join(root, ".sftp-autosync"), { recursive: true });
      writeFileSync(
        join(root, ".sftp-autosync", "status.json"),
      ...
      writeFileSync(
        join(root, ".sftp-autosync", "pending.json"),
        JSON.stringify({
          version: 1,
          ops: [
            {
      ...
      expect(output).toContain("lastError: a.js — scp failed");
      expect(output).toContain("pending: 1 op(s)");
      expect(output).toContain("nextRetry:");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
