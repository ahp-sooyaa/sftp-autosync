...
describe("parsePushArgs", () => {
  ...
  test("parses --changed", async () => {
    const { parsePushArgs } = await import("./commands/push.js");
    expect(parsePushArgs(["--changed"])).toEqual({
      projectDir: null,
      paths: [],
      changed: true,
      force: false,
    ...
    expect(parsePushArgs(["--project", "~/Sites/app", "--changed", "--force"])).toEqual({
      projectDir: "~/Sites/app",
      paths: [],
      changed: true,
      force: true,
  ...
  test("rejects --changed with explicit paths", async () => {
    const { parsePushArgs } = await import("./commands/push.js");
    expect(() => parsePushArgs(["--changed", "index.html"])).toThrow(/without explicit paths/);
  });
